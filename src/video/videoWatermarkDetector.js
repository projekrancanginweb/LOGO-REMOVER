...
function getVideoAlphaMap(size, options = {}) {
    ...
    if (!alphaSource) {
        throw new Error('Peta alfa Gemini 96px tidak ditemukan, tidak dapat membuat cetak biru watermark video');
    }
...
export function detectDiamondVideoWatermarkFromFrames({
    ...
    if (!Array.isArray(frames) || frames.length === 0) {
        throw new Error('Tidak ada frame video yang tersedia untuk dideteksi');
    }
    if (!candidates.length) {
        throw new Error(`Kandidat watermark video ukuran ${width}x${height} belum didukung saat ini`);
    }
