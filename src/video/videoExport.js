...
function createRuntimeCanvas(width, height) {
    ...
    throw new Error('Lingkungan saat ini tidak memiliki Canvas yang tersedia');
}
...
async function getVideoContext(file) {
    ...
    if (!videoTrack) {
        input.dispose();
        throw new Error('Tidak ada trek video yang dapat diproses di dalam file');
    }
...
export async function detectGeminiVideoWatermark(file, options = {}) {
    ...
    try {
        ...
        if (!frames.length) {
            throw new Error('Gagal mengekstrak bingkai deteksi dari video');
        }
...
export async function removeGeminiVideoWatermark(file, options = {}) {
    ...
    if (!detection.isConfident && options.allowLowConfidence !== true) {
        throw new Error('Tingkat keyakinan deteksi watermark video rendah, ekspor dihentikan. Anda dapat mengaktifkan Ekspor Keyakinan Rendah lalu mencoba lagi.');
    }
    ...
    if (!canEncodeAvc) {
        throw new Error('Browser saat ini tidak mendukung encoding WebCodecs H.264/AVC, silakan gunakan Chrome atau Edge versi terbaru.');
    }
    ...
    try {
        ...
        if (!target.buffer) {
            throw new Error('Gagal mengekspor video, hasil output kosong');
        }
